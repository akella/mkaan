...
const OPACITY_STEPS_DESKTOP = [1, 0.4, 0.3, 0.2];
const OPACITY_STEPS_MOBILE = [1, 0.4, 0.3, 0];
...
export default class TextRing {
  ...
  getStepIndex(scrollAngle) {
    ...
    return Math.abs(diff) / stepAngle;
  }

  interpolateSteps(stepIndex, steps) {
    const lastIdx = steps.length - 1;
    if (stepIndex <= 0) return steps[0];
    if (stepIndex >= lastIdx) return steps[lastIdx];
    const lower = Math.floor(stepIndex);
    const t = stepIndex - lower;
    return THREE.MathUtils.lerp(steps[lower], steps[lower + 1], t);
  }

  getOpacityForStep(stepIndex) {
    const steps = this.isMobile() ? OPACITY_STEPS_MOBILE : OPACITY_STEPS_DESKTOP;
    return this.interpolateSteps(stepIndex, steps);
  }

  getBlurForStep(stepIndex) {
    return this.interpolateSteps(stepIndex, BLUR_STEPS);
  }
}
